...
-- Send literal escape to terminal; C-[ no longer works in nvim 0.11+ due to Esc equivalence
vim.keymap.set('t', '<C-[>', function()
  local chan = vim.bo.channel
  if chan ~= 0 then
    vim.api.nvim_chan_send(chan, '\27')
  end
end, { desc = 'Send literal escape to terminal' })
...
vim.keymap.set('n', '<Leader>fD', ':Remove!<CR>', { desc = '[D]elete current file' })
